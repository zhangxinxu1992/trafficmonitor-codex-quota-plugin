...
- Modify `README.md` and `docs/implementation-notes.md`: document Options sign-in, credential storage, and the absence of plaintext token fallback behavior.

## Task 1: Token Source And Device Flow Parsing

- [ ] Write failing tests in `tests/GitHubCopilotQuotaCoreTests.cpp` for token precedence: stored token and missing token.
...
- [ ] Write a failing test showing plaintext config tokens are not used as a fallback.
